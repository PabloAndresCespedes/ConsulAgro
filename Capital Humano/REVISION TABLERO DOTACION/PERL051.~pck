...
create or replace package body PERL051 is
 ...
 BEGIN
   ...
   /*select COUNT(*)
    INTO   V_CONTADOR
   from per_dotacion_jsa F
   WHERE F.DOT_EMPRESA = P_EMPRESA
     AND F.DOT_MES_ANHO = TO_CHAR(LAST_DAY(ADD_MONTHS(P_FECHA, -1)), 'MM/YYYY');

 IF V_CONTADOR = 0 THEN
   insert into per_dotacion_jsa
     (dot_empresa, dot_departamento, dot_legajo, dot_nombre, dot_situacion, dot_ingreso, dot_salida, dot_dpto_real, dot_mes_anho, dot_incluye_com)
     SELECT P_EMPRESA empresa,
                       CASE
                            ...
                            END DEPARTAMENTO,
                                      ...
                                      TO_CHAR(LAST_DAY(ADD_MONTHS(P_FECHA, -1)), 'MM/YYYY'),
                                      E.EMPL_INCLUYE_TVC
                   ...
                   WHERE E.EMPL_DEPARTAMENTO = DPTO.DPTO_CODIGO
                     ...
                     and    CASE
                                  WHEN EMPL_SITUACION = 'I' AND
                                   ...
                                   EMPL_LEGAJO
                                 WHEN EMPL_SITUACION = 'A' AND EMPL_FEC_INGRESO <= LAST_DAY(ADD_MONTHS(P_FECHA, -1)) THEN
                        EMPL_LEGAJO
                           END  IS NOT NULL;

 END IF;

 IF  LAST_DAY(P_FECHA) = P_FECHA THEN
    select COUNT(*)
    INTO   V_CONTADOR
   from per_dotacion_jsa F
   WHERE F.DOT_EMPRESA = P_EMPRESA
     AND F.DOT_MES_ANHO = TO_CHAR(P_FECHA, 'MM/YYYY');
...
------------------------------------------------------------------------------------------------verificado
 ...
 if  P_FECHA = last_day(TO_DATE(P_FECHA))  then
   p_and_transagro := p_and_transagro ||' semana is null and MES || ''/'' || ANHO = '''||V_MES3||'''';
    ...
    P_AND_SOL := P_AND_SOL|| 'and semana is null AND SOL.MES||''/''||SOL.ANHO = '''||V_MES3||'''';
    P_AND_SEL := P_AND_SEL|| 'and semana is null AND SEL.MES||''/''||SEL.ANHO = '''||V_MES3||'''';
...
PROCEDURE PP_PROCESO_SELECCION_CONT(P_EMPRESA IN NUMBER, P_FECHA IN DATE)IS
 ...
 BEGIN
  ...
  P_QUERY2:='SELECT
                 ...
                 TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
              FROM DUAL';
    P_QUERY3 :='/*SELECT ''Re-Contratacion'',6
                  FROM DUAL
                 UNION ALL*/
                 SELECT ''Directa'',4
                  FROM DUAL
 ...
 IF  P_FECHA = LAST_DAY(TO_DATE(P_FECHA))  THEN
    ...
    P_AND_SOL := P_AND_SOL|| 'AND SOL.MES||''/''||SOL.ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')  ';
    P_AND_SEL := P_AND_SEL|| 'AND SEL.MES||''/''||SEL.ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')  ';

 ELSE
   P_AND := P_AND||' AND SEMANA || ''/'' || ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''IW/YYYY'')  ';
    P_AND_SOL := P_AND_SOL|| 'AND SOL.SEMANA||''/''||SOL.ANHO =TO_CHAR(TO_DATE('''||P_FECHA||'''),''IW/YYYY'')  ';
    P_AND_SEL := P_AND_SEL|| 'AND SEL.SEMANA||''/''||SEL.ANHO =TO_CHAR(TO_DATE('''||P_FECHA||'''),''IW/YYYY'')  ';
 END IF;
  ...
  P_QUERY7 := 
              ' SELECT   1 Q_VACANT,
                 ...
                 WHERE SOL.SOLPER_EMPR = 1
                  ...
                  SELECT   1 Q_VACANT,
                     null NRO_SOL,
                     TO_CHAR(PCON_FECHA_INICIO, ''MON/YYYY'') MES_ANHO,
                     AR.AREDPP_DESC AREA,
                     ''Re-Contratacion'' TIPO_CONTRATACION,
                     ''Re-Contratacion'' TIPO_SELEC_SOL,
                     PC.CAR_CODIGO,
                     ADD_MONTHS(TO_DATE(PCON_FECHA_INICIO),-1),
                     NULL,
                     NULL URGENCIA,
                     NULL TIPO_CONTRATO,
                     NULL ESTADO_APROB_SOL,
                     NULL OPERADOR_APROB_SOL,
                     NULL FECHA_APROB_SOL,
                     ''Finalizado'' ESTADO_FIN_SOL,
                      1 ORDEN,
                     TO_CHAR(ADD_MONTHS(TO_DATE(PCON_FECHA_INICIO),-1),''MM/YYYY'') MES_ANHOG
                  FROM PER_AREA_DPP                AR,
                       PER_CARGO                   PC,
                       PER_CONT_SIN_PROSELC       CON
                 WHERE CON.PCON_AREA = AR.AREDPP_CLAVE
                   AND CON.PCON_EMPR = ar.aredpp_empr
                   AND CON.PCON_CARGO = PC.CAR_CODIGO
                   AND CON.PCON_EMPR = PC.CAR_EMPR
                   AND TO_CHAR(CON.PCON_FECHA_INICIO,''MM/YYYY'') =''04/2020''
                   AND ''04/2020'' =  TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''),-2),''MM/YYYY'')
               UNION ALL
                SELECT   1 Q_VACANT,
                     SOL.SOLPER_CLAVE NRO_SOL,
                     TO_CHAR(CONSEL_FECHA_INICIO, ''MON/YYYY'') MES_ANHO,
            ...
            /*    SELECT SOLPER_CANT Q_VACANT,
                     T.SOLPER_CLAVE NRO_SOL,
                     TO_CHAR(SOLPER_FECHA_SOL, ''MON/YYYY'') MES_ANHO,
                     A.AREDPP_DESC AREA,
                     DECODE(SOLPER_TIPO_CONTRATACION,''PR'',''Programada'',''DR'',''Directa'',''RC'',''Re-Contratacion'') TIPO_CONTRATACION,
                     DECODE(SOLPER_TIPO_SELEC,''PR'',''Programada'',''AN'',''Anticipada'', ''RC'', ''Re-Contratacion'')TIPO_SELEC_SOL,
                     T.SOLPER_CARGO,
                     SOLPER_FECHA_SOL,
                     T.SOLPER_OPERADOR_SOL,
                     DECODE(SOLPER_NIVEL_URGENCIA,''M'',''MEDIO'',''A'',''ALTO'',''B'',''BAJO'') URGENCIA,
                     DECODE(SOLPER_TIPO_CONT,''IN'',''Indefinido'',''TE'',''Temporal'') TIPO_CONTRATO,
                     DECODE(SOLPER_ESTADO_APROB,''C'',''Confirmado'',''P'',''Pendiente'',''R'',''Rechazado'',NULL,''Pendiente'') ESTADO_APROB_SOL,
                     SOLPER_OPERADOR_APROB OPERADOR_APROB_SOL,
                     ...
                     DECODE(SOLPER_ESTADO_FINAL,''F'',''Finalizado'',''EP'',''En Proceso'',''R'',''Rechazado'',''P'',''Pendiente'',''EE'',''En Espera'', ''ER'', ''En Reclutamiento'', ''A'',''Anulado'') ESTADO_FIN_SOL,
                     2 ORDEN,
                     TO_CHAR(SOLPER_FECHA_SOL, ''MM/YYYY'') MES_ANHOG
                 FROM PER_SOLICITUD_PERSONAL_HIST T,PER_AREA_DPP A
                 WHERE T.SOLPER_AREA = A.AREDPP_CLAVE
                 AND T.SOLPER_EMPR = A.AREDPP_EMPR
                 AND TO_CHAR(SOLPER_FECHA_SOL, ''MM/YYYY'') =TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''),-1),''MM/YYYY'')
                 AND T.MES || ''/'' || ANHO =TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''),-1),''MM/YYYY'')
                UNION ALL
               SELECT SOLPER_CANT Q_VACANT,
                     T.SOLPER_CLAVE NRO_SOL,
                     TO_CHAR(SOLPER_FECHA_SOL, ''MON/YYYY'') MES_ANHO,
                 ...
                 AND T.SOLPER_EMPR = A.AREDPP_EMPR
                 and SOLPER_TIPO_CONTRATACION <>''RC''
                 AND TO_CHAR(SOLPER_FECHA_SOL, ''MM/YYYY'') = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
                 AND T.SEMANA|| ''/'' || ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''IW/YYYY'')
              */
              union all
               SELECT   1 Q_VACANT,
                     SOL.SOLPER_CLAVE NRO_SOL,
                 ...
                 WHERE SOL.SOLPER_EMPR = 1
                   ...
                   AND SOLPER_ESTADO_FINAL = ''F''
                     '||P_AND_SOL||'
                     '||P_AND_SEL ||'
                  -- AND SOL.mes||''/''||SOL.ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''mm/YYYY'')
                  -- AND SEL.mes||''/''||SEL.ANHO =TO_CHAR(TO_DATE('''||P_FECHA||'''),''mm/YYYY'')
                   AND TO_CHAR(CON.CONSEL_FECHA_INICIO,''MM/YYYY'') = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
                /* UNION ALL
                 SELECT
                     1 VACANCIA,
                     S.PCON_SOL_NRO NRO_SOL,
                     TO_CHAR(S.PCON_FECHA_INICIO, ''MON/YYYY'')MES_ANHO,
                      AREDPP_DESC AREA,
                     ''Re-Contratacion'' TIPO_CONTRATACION,
                     ''Re-Contratacion'' TIPO_SELECCION,
                     S.PCON_CARGO CARGO,
                     S.PCON_FECHA_INICIO FECHA_SOL,
                     NULL,
                     NULL URGENCIA,
                     DECODE(S.PCON_TIPO_CONT,''IN'',''Indefinido'',''TE'',''Temporal'') TIPO_CONTRATO,
                     ''Confirmado''  ESTADO_APROB_SOL,
                     NULL,
                     NULL,
                     ''Finalizado'' ESTADO_FIN_SOL,
                      3 ORDEN,
                      TO_CHAR(S.PCON_FECHA_INICIO, ''MON/YYYY'') MES_ANHOG
                FROM PER_CONT_SIN_PROSELC S, PER_SOLICITUD_PERSONAL G,PER_AREA_DPP dd
               WHERE S.PCON_SOL_NRO = G.SOLPER_CLAVE(+)
                 AND S.PCON_EMPR = G.SOLPER_EMPR(+)
                 AND DD.AREDPP_CLAVE = S.PCON_AREA
                 AND DD.AREDPP_EMPR = S.PCON_EMPR
                 AND S.PCON_EMPR = 1
                 AND TO_CHAR(PCON_FECHA_INICIO,''MM/YYYY'') =TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
                 AND G.SOLPER_CLAVE IS not NULL */
                 UNION ALL
                SELECT
                     1 VACANCIA,
                     S.PCON_SOL_NRO NRO_SOL,
                     TO_CHAR(ADD_MONTHS(S.PCON_FECHA_INICIO, -1), ''MON/YYYY'')MES_ANHO,
                      AREDPP_DESC AREA,
                     ''Re-Contratacion'' TIPO_CONTRATACION,
                     ''Re-Contratacion'' TIPO_SELECCION,
                     S.PCON_CARGO CARGO,
                     ADD_MONTHS(S.PCON_FECHA_INICIO, -1) FECHA_SOL,
                     NULL,
                     NULL URGENCIA,
                     DECODE(S.PCON_TIPO_CONT,''IN'',''Indefinido'',''TE'',''Temporal'') TIPO_CONTRATO,
                     ''Confirmado''  ESTADO_APROB_SOL,
               ...
               WHERE S.PCON_SOL_NRO = G.SOLPER_CLAVE(+)
                 ...
                 AND DD.AREDPP_EMPR = S.PCON_EMPR
                 AND S.PCON_EMPR = 1
                 AND TO_CHAR(ADD_MONTHS(S.PCON_FECHA_INICIO, -1), ''MM/YYYY'') = ''03/2020''
                 AND ''03/2020'' =  TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''),-2),''MM/YYYY'')
                 AND G.SOLPER_CLAVE IS NULL';
...
/*
  P_QUERY2:= 'SELECT CASE
               WHEN MES_ANHO = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -2),''MM/YYYY'') THEN
               sum(Q_VACANT)
               END MES1,
               CASE
               WHEN MES_ANHO = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'') THEN
               sum(Q_VACANT)
               END MES2,
               CASE
               WHEN MES_ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'') THEN
               sum(Q_VACANT)
               END MES3,
               TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -2),''MM/YYYY'')MES_1,
               TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'') MES_2,
               TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'') MES_3
          FROM (SELECT Q_VACANT, NRO_SOL, to_char(FECHA_SOL,''MM/YYYY'') MES_ANHO
                 FROM CUBO_LONDON_PROCESO_SELEC V
                 WHERE FECHA_SOL BETWEEN
                       ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
                       TO_DATE('''||P_FECHA||''')
                 GROUP BY to_char(FECHA_SOL,''MM/YYYY''), FECHA_APROB_SOL, NRO_SOL, Q_VACANT
                 UNION ALL
                 select 1, NULL, to_char(pcon_fecha_inicio, ''MM/YYYY'')
                     from per_cont_sin_proselc
                    where pcon_empr = 1
                      and to_char(pcon_fecha_inicio, ''MM/YYYY'')<>( TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY''))
                     )
         GROUP BY MES_ANHO';
...
SELECT SOLPER_CANT Q_VACANT,
   ...
   WHERE T.SOLPER_AREA = A.AREDPP_CLAVE
   AND T.SOLPER_EMPR = A.AREDPP_EMPR
   AND TO_CHAR(SOLPER_FECHA_SOL, 'MM/YYYY') = TO_CHAR(ADD_MONTHS(&V_FECHA3, -2), 'MM/YYYY')
   AND T.MES || '/' || ANHO = '03/2020'
 UNION ALL
  SELECT SOLPER_CANT Q_VACANT,
       T.SOLPER_CLAVE NRO_SOL,
       TO_CHAR(SOLPER_FECHA_SOL, 'MON/YYYY') MES_ANHO,
   ...
   WHERE T.SOLPER_AREA = A.AREDPP_CLAVE
   AND T.SOLPER_EMPR = A.AREDPP_EMPR
   AND TO_CHAR(SOLPER_FECHA_SOL, 'MM/YYYY') = TO_CHAR(ADD_MONTHS(&V_FECHA3, -1), 'MM/YYYY')
   AND T.MES || '/' || ANHO = '04/2020'
  UNION ALL
  SELECT SOLPER_CANT Q_VACANT,
       T.SOLPER_CLAVE NRO_SOL,
       TO_CHAR(SOLPER_FECHA_SOL, 'MON/YYYY') MES_ANHO,
   ...
   WHERE T.SOLPER_AREA = A.AREDPP_CLAVE
   AND T.SOLPER_EMPR = A.AREDPP_EMPR
   AND TO_CHAR(SOLPER_FECHA_SOL, 'MM/YYYY') = TO_CHAR(TO_DATE(&V_FECHA3,'DD/MM/YYYY'), 'MM/YYYY')
   AND T.SEMANA|| '/' || ANHO = '21/2020'
   ...
   SELECT S.PCON_SOL_NRO NRO_SOL,
       DECODE(G.SOLPER_FECHA_SOL,
              NULL,
              TO_CHAR(ADD_MONTHS(S.PCON_FECHA_INICIO, -1), 'MM/YYYY'),
              TO_CHAR(SOLPER_FECHA_SOL, 'MM/YYYY')) MES_ANHO,
       DECODE(G.SOLPER_FECHA_SOL,
              NULL,
              ADD_MONTHS(S.PCON_FECHA_INICIO, -1),
              SOLPER_FECHA_SOL) FECHA_SOL,
       G.SOLPER_OPERADOR_SOL,
       G.SOLPER_NIVEL_URGENCIA URGENCIA,
       DECODE(G.SOLPER_AREA, NULL, S.PCON_AREA, SOLPER_AREA) AREA,
       DECODE(G.SOLPER_CARGO, NULL, S.PCON_CARGO, SOLPER_CARGO) CARGO,

 DECODE(DECODE(G.SOLPER_TIPO_CONT,
              NULL,
              S.PCON_TIPO_CONT,
              G. SOLPER_TIPO_CONT),'IN','Indefinido','TE','Temporal') TIPO_CONTRATO,
       'Re-Contratacion' TIPO_CONTRATACION,
       DECODE(SOLPER_ESTADO_APROB,'C','Confirmado','P','Pendiente','R','Rechazado',NULL,'Pendiente') ESTADO_APROB_SOL,
       NVL(G.SOLPER_CANT, 1) VACANCIA,
       G.SOLPER_OPERADOR_APROB,
       G.SOLPER_FECHA_APROB,
       'Re-Contratacion' TIPO_SELECCION,
       DECODE(SOLPER_ESTADO_FINAL,'F','Finalizado','EP','En Proceso','R','Rechazado','P','Pendiente','EE','En Espera') ESTADO_FIN_SOL,
  DECODE (PCON_SOL_NRO, NULL, S.PCON_LEGAJO) LEGAJO,
  DECODE (G.SOLPER_EMPR, NULL, S.PCON_EMPR) EMPR
  FROM PER_CONT_SIN_PROSELC S, PER_SOLICITUD_PERSONAL G
 WHERE S.PCON_SOL_NRO = G.SOLPER_CLAVE(+)
   AND S.PCON_EMPR = G.SOLPER_EMPR(+)
   AND S.PCON_EMPR = 1
   AND G.SOLPER_CLAVE IS NULL---
        ...
        P_QUERY:='SELECT SUM(C001) VACA,
                ...
                C016 ORDEN,
                 CASE
                    ...
                    SUM(C001)
                 --   WHEN C005 = ''Re-Contratacion'' AND C016 = 1 THEN
                 --   SUM(C001)
                ...
                CASE
                    ...
                    SUM(C001)
             --       WHEN C005 = ''Re-Contratacion'' AND C016 = 2 THEN
                --    SUM(C001)
                ...
                CASE
                    ...
                    SUM(C001)
                  --  WHEN C005 = ''Re-Contratacion'' AND C016 = 3 THEN
                  --  SUM(C001)
...
PROCEDURE PP_PROCESO_SELECCION_SELC(P_EMPRESA IN NUMBER, P_FECHA IN DATE)IS
 ...
 /* P_QUERY := 'SELECT
              ----TIPO_SELECCION
               ...
               MES_ANHO,
               TIPO_SELEC_SOL,
               SUM(SUM(Q_VACANT)) OVER(PARTITION BY AREA, MES_ANHO ORDER BY MES_ANHO) TOTAL_VAC_AREA,
               ROUND(SUM(Q_VACANT) /
                     NVL(SUM(SUM(Q_VACANT))
                         OVER(PARTITION BY AREA, MES_ANHO ORDER BY MES_ANHO),
                         1) * 100) POR_SELEC,
               ----TIPO_CONTRATACION
               TIPO_CONTRATACION,
               SUM(SUM(Q_VACANT)) OVER(PARTITION BY TIPO_CONTRATACION, MES_ANHO ORDER BY MES_ANHO) TOTAL_TIPO_CONT,
               ROUND(SUM(Q_VACANT) /
                     NVL(SUM(SUM(Q_VACANT))
                         OVER(PARTITION BY TIPO_CONTRATACION, MES_ANHO ORDER BY MES_ANHO),
                         1) * 100) POR_CONT,
               COUNT(Q_VACANT),
               ORDEN,
               CASE
                 WHEN TIPO_SELEC_SOL = ''Anticipada'' AND ORDEN = 1 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_SELEC_SOL = ''Programada'' AND ORDEN = 1 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_SELEC_SOL = ''Re-contratacion'' AND ORDEN = 1 THEN
                  SUM(Q_VACANT)
               END MES1_ant,
               CASE
                 WHEN TIPO_SELEC_SOL = ''Anticipada'' AND ORDEN = 2 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_SELEC_SOL = ''Programada'' AND ORDEN = 2 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_SELEC_SOL = ''Re-contratacion'' AND ORDEN = 2 THEN
                  SUM(Q_VACANT)
               END MES2_ant,
               CASE
                 WHEN TIPO_SELEC_SOL = ''Anticipada'' AND ORDEN = 3 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_SELEC_SOL = ''Programada'' AND ORDEN = 3 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_SELEC_SOL = ''Re-contratacion'' AND ORDEN = 3 THEN
                  SUM(Q_VACANT)
               END MES3_ant,
               CASE
                 WHEN TIPO_CONTRATACION = ''Directa'' AND ORDEN = 1 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_CONTRATACION = ''Programada'' AND ORDEN = 1 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_CONTRATACION = ''Re-contratacion'' AND ORDEN = 1 THEN
                  SUM(Q_VACANT)
               END MES1_CONT,
               CASE
                 WHEN TIPO_CONTRATACION = ''Directa'' AND ORDEN = 2 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_SELEC_SOL = ''Programada'' AND ORDEN = 2 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_CONTRATACION = ''Re-contratacion'' AND ORDEN = 2 THEN
                  SUM(Q_VACANT)
               END MES2_CONT,
               CASE
                 WHEN TIPO_CONTRATACION = ''Directa'' AND ORDEN = 3 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_CONTRATACION = ''Programada'' AND ORDEN = 3 THEN
                  SUM(Q_VACANT)
                 WHEN TIPO_CONTRATACION = ''Re-contratacion'' AND ORDEN = 3 THEN
                  SUM(Q_VACANT)
               END MES3_CONT
                FROM (SELECT 1 Q_VACANT,
                             NRO_SOL,
                             to_char(v.FECHA_SOL, ''MM/YYYY'') MES_ANHO,
                             AREA_DESC AREA,
                             TIPO_SELEC_SOL,
                             TIPO_CONTRATACION,
                             CASE
                               WHEN to_char(v.FECHA_SOL, ''MM/YYYY'') =
                       ...
                       WHERE FECHA_SOL BETWEEN
                         ...
                         AND V.COD_POSTULANTE IS NOT NULL
                       GROUP BY to_char(v.FECHA_SOL, ''MM/YYYY''),
                                FECHA_SOL,
                                NRO_SOL,
                                Q_VACANT,
                                AREA_DESC,
                      ...
                      SELECT 1,
                             ...
                             DECODE(G.SOLPER_FECHA_SOL,
                                    NULL,
                                    TO_CHAR(ADD_MONTHS(S.PCON_FECHA_INICIO,-2), ''MM/YYYY''),
                                    TO_CHAR(SOLPER_FECHA_SOL, ''MM/YYYY'')),
                             X.AREDPP_DESC,
                             ''Re-contratacion'',
                             ''Re-contratacion'',
                             CASE
                               WHEN TO_CHAR(PCON_FECHA_INICIO, ''MM/YYYY'') =
                                    TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -2), ''MM/YYYY'') THEN
                                1
                               WHEN TO_CHAR(PCON_FECHA_INICIO, ''MM/YYYY'') =
                                    TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1), ''MM/YYYY'') THEN
                             ...
                             END ORDEN,
                             S.PCON_LEGAJO
                        FROM PER_CONT_SIN_PROSELC   S,
                             PER_AREA_DPP           X,
                             PER_SOLICITUD_PERSONAL G
                       WHERE S.PCON_AREA = X.AREDPP_CLAVE
                         AND S.PCON_EMPR = X.AREDPP_EMPR
                         AND S.PCON_SOL_NRO = G.SOLPER_CLAVE(+)
                         AND S.PCON_EMPR = G.SOLPER_EMPR(+)
                         AND DECODE(G.SOLPER_FECHA_SOL,
                                    NULL,
                                    PCON_FECHA_INICIO,
                                    SOLPER_FECHA_SOL) BETWEEN
                             ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
                             TO_DATE('''||P_FECHA||''')
                         AND PCON_EMPR = 1)
               GROUP BY AREA, MES_ANHO, TIPO_SELEC_SOL, TIPO_CONTRATACION, ORDEN';*/
...
P_QUERY7 := '/*SELECT SOLPER_CANT Q_VACANT,
                     T.SOLPER_CLAVE NRO_SOL,
                     TO_CHAR(SOLPER_FECHA_SOL, ''MON/YYYY'') MES_ANHO,
                     A.AREDPP_DESC AREA,
                     DECODE(SOLPER_TIPO_CONTRATACION,''PR'',''Programada'',''DR'',''Directa'',''RC'',''Re-Contratacion'') TIPO_CONTRATACION,
                 ...
                 AND T.SOLPER_EMPR = A.AREDPP_EMPR
                 and SOLPER_TIPO_CONTRATACION <>''RC''
                 AND TO_CHAR(SOLPER_FECHA_SOL, ''MM/YYYY'') = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''),-2),''MM/YYYY'')
                 AND T.MES || ''/'' || ANHO = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''),-2),''MM/YYYY'')*/
               ...
               UNION ALL
                ...
                SELECT SOLPER_CANT Q_VACANT,
                 ...
                 and SOLPER_TIPO_CONTRATACION <>''RC''
                 AND TO_CHAR(SOLPER_FECHA_SOL, ''MM/YYYY'') = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
                 AND T.mes|| ''/'' || ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''mm/YYYY'')*/
                UNION ALL
                  SELECT   1 Q_VACANT,
                     SOL.SOLPER_CLAVE NRO_SOL,
                     TO_CHAR(seleper_fecha_estado_gral, ''MON/YYYY'') MES_ANHO,
  ...
  /*P_QUERY:= 'SELECT
            ----TIPO_SELECCION
             ...
             END MES3_ant
              ...
              FROM (SELECT Q_VACANT,
                           NRO_SOL,
                           to_char(v.FECHA_SOL, ''MON/YYYY'') MES_ANHO,
                           AREA_DESC AREA,
                           TIPO_SELEC_SOL,
                           TIPO_CONTRATACION,
                           CASE
                     ...
                     WHERE FECHA_SOL BETWEEN
                           ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
                           TO_DATE('''||P_FECHA||''')
                           AND TIPO_SELEC_SOL <>''Re-Contratacion''
                     GROUP BY to_char(v.FECHA_SOL, ''MM/YYYY''),
                              FECHA_SOL,
                              NRO_SOL,
                              Q_VACANT,
                    ...
                    UNION ALL
                    SELECT NVL(G.SOLPER_CANT,1) VACANCIA,
                           S.PCON_SOL_NRO,
                           DECODE(G.SOLPER_FECHA_SOL,
                                  NULL,
                                  TO_CHAR(ADD_MONTHS(S.PCON_FECHA_INICIO,-1), ''MON/YYYY''),
                                  TO_CHAR(SOLPER_FECHA_SOL, ''MON/YYYY'')),
                           X.AREDPP_DESC,
                           ''Re-Contratacion'',
                           ''Re-Contratacion'',
                           CASE
                             WHEN TO_CHAR(DECODE(G.SOLPER_FECHA_SOL,
                                  NULL,
                                   ADD_MONTHS(S.PCON_FECHA_INICIO, -1),
                                  SOLPER_FECHA_SOL), ''MM/YYYY'') =
                                  TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -2), ''MM/YYYY'') THEN
                              1
                             WHEN TO_CHAR( DECODE(G.SOLPER_FECHA_SOL,
                                  NULL,
                     ...
                     WHERE S.PCON_AREA = X.AREDPP_CLAVE
                       ...
                       AND DECODE(G.SOLPER_FECHA_SOL,
                                  NULL,
                                  ADD_MONTHS(PCON_FECHA_INICIO,-1),
                                  SOLPER_FECHA_SOL) BETWEEN
                           ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
                           TO_DATE('''||P_FECHA||''')
                       AND PCON_EMPR = 1)
...
*/
        ...
        P_QUERY:='SELECT SUM(C001) VACA,
                ...
                C016 ORDEN,
                 CASE
                    ...
                    SUM(C001)
                 --   WHEN C006 = ''Re-Contratacion'' AND C016 = 1 THEN
                  --  SUM(C001)
                ...
                CASE
                    ...
                    SUM(C001)
                 --   WHEN C006 = ''Re-Contratacion'' AND C016 = 2 THEN
                --    SUM(C001)
                ...
                CASE
                    ...
                    SUM(C001)
               --     WHEN C006 = ''Re-Contratacion'' AND C016 = 3 THEN
                 --   SUM(C001)
 ...
 IF P_FECHA = LAST_DAY(TO_DATE(P_FECHA))  THEN
    ...
    P_AND_T := P_AND_T||' AND T.MES || ''/'' || T.ANHO = '''||V_MES3||'''';
    P_AND_P := P_AND_P||' AND P.MES || ''/'' || P.ANHO = '''||V_MES3||'''';
    P_AND_SOL := P_AND_SOL|| 'AND SOL.MES||''/''||SOL.ANHO = '''||V_MES3||'''';
    P_AND_SEL := P_AND_SEL|| 'AND SEL.MES||''/''||SEL.ANHO = '''||V_MES3||'''';

 ELSE
    P_AND_T := P_AND_T||' AND T.MES || ''/'' || T.ANHO = '''||V_SEMANA||'''';
    P_AND_P := P_AND_P||' AND P.MES || ''/'' || P.ANHO = '''||V_SEMANA||'''';
    P_AND_SOL := P_AND_SOL|| 'AND SOL.SEMANA||''/''||SOL.ANHO ='''||V_SEMANA||'''';
    P_AND_SEL := P_AND_SEL|| 'AND SEL.SEMANA||''/''||SEL.ANHO ='''||V_SEMANA||'''';
 END IF;
  ...
  P_QUERY20 := 'SELECT P.PER_ENT_CODIGO,
                  ...
                  SELECT P.PER_ENT_CODIGO,
                   ...
                   WHERE T.ENTPER_ENTREVISTA = P.PER_ENT_CODIGO
                      ...
                      AND ENTPER_EMPR = 1
                        '||P_AND_T||'
                        '||P_AND_P||'
                     --- AND T.mes||''/''||T.ANHO = '''||V_MES3||'''
                    --- AND P.mes||''/''||P.ANHO = '''||V_MES3||'''
                ...
                ---   AND SOL.mes||''/''||SOL.ANHO ='''||V_MES3||'''
                ---   AND SEL.mes||''/''||SEL.ANHO ='''||V_MES3||'''
                   AND TO_CHAR(SELEPER_REQUIS_FECHA, ''mm/yyyy'') ='''||V_MES3||'''
                  -- AND CASE WHEN SELEPER_REQUIS_FECHA >''01/05/2020'' AND AREDPP_DESC <> ''CDA''  THEN 0
                   --        WHEN SELEPER_REQUIS_FECHA >''01/06/2020'' AND AREDPP_DESC = ''CDA''  THEN   0
                   --        ELSE 1 END = 1
                ...
                --   AND CASE WHEN SELEPER_REF_FECHA >''01/05/2020'' AND AREDPP_DESC <> ''CDA''  THEN 0
                 --      WHEN SELEPER_REF_FECHA >''01/06/2020'' AND AREDPP_DESC = ''CDA''  THEN   0
                --       ELSE 1 END = 1
...
/*
 P_QUERY2 := 'SELECT DETALLE,
       CANTIDAD,
       CASE WHEN MES_ANHO = TO_CHAR(ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2),''MM/YYYY'') THEN
         CANTIDAD
         END MES1,
        CASE WHEN MES_ANHO = TO_CHAR(ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -1),''MM/YYYY'') THEN
         CANTIDAD
         END MES2,
       CASE WHEN MES_ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'') THEN
         CANTIDAD
         END MES3,
       ORDEN
FROM(
SELECT ''1RA. ENTREVISTA'' DETALLE, COUNT (NOMBRE_POST) CANTIDAD, MES_ANHO, 1 ORDEN
                FROM
                (SELECT  NOMBRE_POST,TO_CHAR(FECHA_REQUIS, ''MM/YYYY'') MES_ANHO,AREA_DESC, ESTADO
                FROM
                CUBO_LONDON_PROCESO_SELEC T
                WHERE NOMBRE_POST <>'' ''
                AND T.FECHA_REQUIS BETWEEN ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND  TO_DATE('''||P_FECHA||''')
                AND ETAPA = ''1 -REQUISITOS''
                AND CASE WHEN FECHA_REQUIS >''01/05/2020'' AND AREA_DESC <> ''CDA''  THEN 0
                         WHEN FECHA_REQUIS >''01/06/2020'' AND AREA_DESC = ''CDA''  THEN   0
                         ELSE 1 END = 1
                ...
                GROUP BY MES_ANHO
           UNION ALL
           SELECT ''REFERENCIAS'' DETALLE, COUNT (NOMBRE_POST) CANTIDAD, MES_ANHO,4
                FROM
                (SELECT  NOMBRE_POST,TO_CHAR(FECHA_REF, ''MM/YYYY'') MES_ANHO,AREA_DESC, ESTADO
                FROM
                CUBO_LONDON_PROCESO_SELEC T
                WHERE NOMBRE_POST <>'' ''
                AND t.FECHA_REF BETWEEN ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND  TO_DATE('''||P_FECHA||''')
                AND ETAPA = ''4 -REFERENCIAS''
                AND ESTADO_APROB_SOL <> ''Rechazado''
                AND CASE WHEN t.FECHA_REF >''01/05/2020'' AND AREA_DESC <> ''CDA''  THEN 0
                         WHEN FECHA_REF >''01/06/2020'' AND AREA_DESC = ''CDA'' THEN  0
                         ELSE 1 END = 1
                UNION ALL
                SELECT TO_CHAR(P.PER_ENT_CODIGO), TO_CHAR(T.ENTPER_REF_FECHA, ''mm/yyyy''), S.AREDPP_DESC,
                       DECODE(t.entper_ref_estado, ''C'', ''Cerrado'', ''A'', ''Abierto'')
                  FROM PER_ENTREVISTA_PERSONAL T, PER_ENTREVISTA_POST P , PER_CARGO_DPP C,  PER_AREA_DPP S
                 WHERE T.ENTPER_ENTREVISTA = P.PER_ENT_CODIGO
                    AND T.ENTPER_EMPR = P.PER_ENT_EMPR
                    ...
                    AND C.CARDPP_AREA = S.AREDPP_CLAVE
                    AND C.CARDPP_EMPR = S.AREDPP_EMPR
                    AND TRUNC(T.ENTPER_REF_FECHA) BETWEEN
                       ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
                       TO_DATE('''||P_FECHA||''')
                )
                GROUP BY MES_ANHO
           UNION ALL
         SELECT ''ENTREVISTA AREA'' DETALLE, COUNT (NOMBRE_POST) CANTIDAD, MES_ANHO, 7
                FROM
                (SELECT NOMBRE_POST,TO_CHAR(FECHA_EAR, ''MM/YYYY'') MES_ANHO,AREA_DESC, ESTADO
                FROM
                CUBO_LONDON_PROCESO_SELEC T
                WHERE NOMBRE_POST <>'' ''
                AND t.FECHA_EAR BETWEEN ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND  TO_DATE('''||P_FECHA||''')
                AND ETAPA = ''5 -ENTREVISTA AREA''
                AND ESTADO_APROB_SOL <> ''Rechazado'') GROUP BY MES_ANHO
            UNION ALL
              SELECT ''ENTREVISTA ESPECIAL'' DETALLE, COUNT (NOMBRE_POST) CANTIDAD, MES_ANHO, 10
                FROM
                (SELECT  NOMBRE_POST,TO_CHAR(FECHA_EES, ''MM/YYYY'') MES_ANHO,AREA_DESC, ESTADO
                FROM
                CUBO_LONDON_PROCESO_SELEC T
                WHERE NOMBRE_POST <>'' ''
                AND t.FECHA_EES BETWEEN ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND  TO_DATE('''||P_FECHA||''')
                AND ETAPA = ''6 -ENTREVISTA ESPECIAL''
                AND ESTADO_APROB_SOL <> ''Rechazado'') GROUP BY MES_ANHO
              UNION ALL
SELECT AREA DETALLE, -----------------en lista entrevista de especial
       COUNT(SELPER_POSTUL) CANT,
       FECHA_EAR,
       12 orden
  FROM (SELECT SOL.SOLPER_CLAVE NRO_SOL,
               CASE
                 WHEN AREDPP_DESC IN (''GERENCIA GENERAL'', ''ADMINISTRATIVA'') THEN
                  ''ADM''
                 ELSE
               ...
               SEL.SELPER_POSTUL,
               TO_CHAR(SEL.SELEPER_EAR_FECHA, ''MM/YYYY'') FECHA_EAR,
               12 orden
          FROM PER_SOLICITUD_PERSONAL SOL,
               PER_AREA_DPP           AR,
               PER_SELECCION_PERSONAL SEL
         WHERE SOL.SOLPER_EMPR = 1
           AND SOL.SOLPER_EMPR = AR.AREDPP_EMPR
           AND SOL.SOLPER_AREA = AR.AREDPP_CLAVE
           AND SOL.SOLPER_EMPR = SEL.SELEPER_EMPR
           AND SOL.SOLPER_CLAVE = SEL.SELPER_SOLICI
           AND SEL.SELEPER_EAR_FECHA BETWEEN
               ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
               TO_DATE('''||P_FECHA||''')
           AND SELEPER_EAR_ACCION = ''PS''
           AND SEL.SELEPER_EAR_ESTADO = ''C''
           AND SEL.SELEPER_EES_FECHA IS NULL) A,
       (SELECT AREA
          FROM (SELECT ''CDA'' AREA FROM DUAL
                UNION ALL
                SELECT ''ADM'' AREA FROM DUAL
                UNION ALL
                SELECT ''COMERCIAL'' AREA FROM DUAL
                UNION ALL
                SELECT ''INDUSTRIAL'' AREA FROM DUAL)) B
 WHERE B.AREA = AREA_DESC(+)
 GROUP BY AREA, FECHA_EAR
 union all-----------------------lista entrevista de area
SELECT AREA DETALLE,
       COUNT(SELPER_POSTUL) CANT,
       FECHA_EAR,
       9 orden
  FROM (SELECT SOL.SOLPER_CLAVE NRO_SOL,
               ...
               SEL.SELPER_POSTUL,
               TO_CHAR(SEL.SELEPER_REF_FECHA, ''MM/YYYY'') FECHA_EAR
          FROM PER_SOLICITUD_PERSONAL SOL,
               PER_AREA_DPP           AR,
               PER_SELECCION_PERSONAL SEL
         WHERE SOL.SOLPER_EMPR = 1
           ...
           AND SOL.SOLPER_EMPR = SEL.SELEPER_EMPR
           AND SOL.SOLPER_CLAVE = SEL.SELPER_SOLICI
           AND SEL.SELEPER_REF_FECHA BETWEEN
               ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
               TO_DATE('''||P_FECHA||''')
           AND sel.seleper_ref_accion = ''PS''
           AND SEL.SELEPER_EAR_FECHA IS NULL
           and sel.seleper_ref_estado= ''C''
           AND SOL.SOLPER_ESTADO_FINAL = ''EP''
           AND NVL(SELEPER_ESTADO_GRAL, ''EP'') <> ''D'') A,
       (SELECT AREA
          FROM (SELECT ''CDA'' AREA FROM DUAL
                UNION ALL
                SELECT ''ADM'' AREA FROM DUAL
 ...
 SELECT AREA DETALLE,
  ...
  FROM (SELECT SOL.SOLPER_CLAVE NRO_SOL,
               ...
               SEL.SELPER_POSTUL,
               TO_CHAR(SEL.SELEPER_PRES_FECHA, ''MM/YYYY'') FECHA_REF
          FROM PER_SOLICITUD_PERSONAL SOL,
               PER_AREA_DPP           AR,
               PER_SELECCION_PERSONAL SEL
         WHERE SOL.SOLPER_EMPR = 1
           ...
           AND SOL.SOLPER_EMPR = SEL.SELEPER_EMPR
           AND SOL.SOLPER_CLAVE = SEL.SELPER_SOLICI
           AND SEL.SELEPER_PRES_FECHA BETWEEN
               ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
               TO_DATE('''||P_FECHA||''')
           AND SEL.SELEPER_PRES_ACCION = ''PS''
           AND SEL.SELEPER_PRES_ESTADO = ''C''
           AND SEL.SELEPER_REF_FECHA IS NULL
           AND CASE
                 WHEN SELEPER_PRES_FECHA > ''01/05/2020'' AND
                      AR.AREDPP_DESC <> ''CDA'' THEN
                  0
                 WHEN SELEPER_PRES_FECHA > ''01/06/2020'' AND
                      AREDPP_DESC = ''CDA'' THEN
                  0
                 ELSE
                  1
               END = 1
        UNION ALL
        SELECT TO_CHAR(P.PER_ENT_CODIGO),
               CASE
                 WHEN AREDPP_DESC IN (''GERENCIA GENERAL'', ''ADMINISTRATIVA'') THEN
                  ''ADM''
                 ELSE
               ...
               PER_ENT_CODIGO,
               TO_CHAR(T.ENTPER_REQUIS_FECHA, ''mm/yyyy'')
          FROM PER_ENTREVISTA_PERSONAL T,
               PER_ENTREVISTA_POST     P,
               PER_CARGO_DPP           C,
               PER_AREA_DPP            S
         WHERE T.ENTPER_ENTREVISTA = P.PER_ENT_CODIGO
           AND T.ENTPER_EMPR = P.PER_ENT_EMPR
           ...
           AND C.CARDPP_AREA = S.AREDPP_CLAVE
           AND C.CARDPP_EMPR = S.AREDPP_EMPR
           AND TRUNC(T.ENTPER_PRES_FECHA) BETWEEN
               ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
               TO_DATE('''||P_FECHA||''')
           AND T.ENTPER_PRES_ACCION = ''PS''
           AND T.ENTPER_PRES_ESTADO = ''C''
           AND T.ENTPER_REF_FECHA IS NULL
       ...
       (SELECT AREA
          FROM (SELECT ''CDA'' AREA FROM DUAL
                ...
                SELECT ''INDUSTRIAL'' AREA FROM DUAL)) B
 WHERE B.AREA = AREA_DESC(+)
 GROUP BY AREA, FECHA_REF
UNION ALL
SELECT AREA DETALLE, -----------------en fila primera entrevista
       COUNT(SELPER_POSTUL) CANT,
       FECHA_SOL,
       3 orden
  FROM (SELECT SOL.SOLPER_CLAVE NRO_SOL,
               ...
               SEL.SELPER_POSTUL,
               TO_CHAR(solper_fecha_sol, ''MM/YYYY'') FECHA_SOL
          FROM PER_SOLICITUD_PERSONAL SOL,
               PER_AREA_DPP           AR,
               PER_SELECCION_PERSONAL SEL
         WHERE SOL.SOLPER_EMPR = 1
           ...
           AND SOL.SOLPER_EMPR = SEL.SELEPER_EMPR
           AND SOL.SOLPER_CLAVE = SEL.SELPER_SOLICI
           AND sol.solper_fecha_sol BETWEEN
               ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
               TO_DATE('''||P_FECHA||''')
           AND SEL.SELEPER_REQUIS_FECHA IS NULL
           AND SOL.SOLPER_ESTADO_FINAL = ''EP''
           AND SEL.SELEPER_ESTADO_GRAL = ''EP''
           AND CASE
                 WHEN sol.solper_fecha_sol > ''01/05/2020'' AND
                      AR.AREDPP_DESC <> ''CDA'' THEN
                  0
                 WHEN sol.solper_fecha_sol > ''01/06/2020'' AND
                      AREDPP_DESC = ''CDA'' THEN
                  0
                 ELSE
                  1
        ...
        SELECT TO_CHAR(P.PER_ENT_CODIGO),
         ...
         WHERE T.ENTPER_ENTREVISTA = P.PER_ENT_CODIGO
           ...
           AND C.CARDPP_AREA = S.AREDPP_CLAVE
           AND C.CARDPP_EMPR = S.AREDPP_EMPR
           AND TRUNC(P.PER_ENT_FECHA) BETWEEN
               ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
               TO_DATE('''||P_FECHA||''')
           AND T.ENTPER_REQUIS_FECHA IS NULL
  ...
  /* P_QUERY7:= 'SELECT EMPL_LEGAJO,NOMBRE,INGRESO, SALIDA, SUCURSAL, DEPARTAMENTO,
    ...
    FROM
                   (SELECT EMPL_LEGAJO,
                         ...
                         EMPL_CARGO CARGO,
                         DECODE(empl_tipo_desv,''DJ'',''Despido Justificado'',''DI'',''Despido Injustificado'',''RV'',''Renuncia Voluntaria'',''TC'',''Termino de Contrato'',''JU'',''Judicial'')TIPO_DESV,
                         (SELECT FAC.FACDES_DESC FROM PER_FACTOR_DESVINCULACION FAC WHERE FAC.FACDES_EMPR = 1 AND FAC.FACDES_CLAVE = DESV.TIPDESV_FACTOR)FACTOR_DESCV,
                         (SELECT MOT.MOTDESV_DESC FROM PER_MOTIVO_DESVINCULACION MOT WHERE MOT.MOTDESV_EMPR = 1 AND MOT.MOTDESV_CLAVE = DESV.TIPDESV_MOTIVO)MOTIVO_DESCV,
                         DESV.TIPDESV_DESC,
                         EM.EMPL_MOTIVO_SALIDA Motivo,
                         DECODE(EM.EMPL_TIPO_CONT,''I'',''INDEFINIDO'',''T'',''TEMPORAL'') TIPO_CONTRATO,
                         TO_CHAR(EMPSOL_FECHA_VERIF, ''MM/YYYY'') MES,
                         TO_CHAR(EMPSOL_FECHA_VERIF, ''IW/YYYY'') SEMANA_ANHO,
                         TO_NUMBER(TO_CHAR(EMPSOL_FECHA_VERIF, ''IW'')) SEMANA,
                          CASE WHEN TO_CHAR(PDOC_FEC_GRAB, ''MM/YYYY'') =  TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'') THEN
                              PDDET_IMP
                              END IMPORTE,
                         CASE WHEN TO_CHAR(PDOC_FEC_GRAB, ''MM/YYYY'') =  TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'') THEN
                   ...
                   WHERE EMPL_SITUACION = ''I''
                     ...
                     AND EM.EMPL_LEGAJO = X.EMPSOL_EMPL_LEG
                     AND EM.EMPL_EMPRESA = X.EMPSOL_EMPR
                     AND X.EMPSOL_ESTADO_EMP_SOL = ''I''
                     AND X.EMPSOL_ESTADO_VERIF = ''C''

                     AND TO_CHAR(X.EMPSOL_FECHA_VERIF, ''MM/YYYY'') = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
                     AND TO_CHAR(PDOC_FEC_GRAB, ''MM/YYYY'') = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
              ...
              /*       SELECT EMPL_LEGAJO,
                         ...
                         TO_CHAR(EMPSOL_FECHA_VERIF, ''IW/YYYY'') SEMANA_ANHO,
                         TO_NUMBER(TO_CHAR(EMPSOL_FECHA_VERIF, ''IW'')) SEMANA,
                          CASE WHEN TO_CHAR(PDOC_FEC_GRAB, ''MM/YYYY'') =   TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'') THEN
                              PDDET_IMP
                              END IMPORTE,
                         CASE WHEN TO_CHAR(PDOC_FEC_GRAB, ''MM/YYYY'') =   TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'') THEN
                         PDDET_CLAVE_CONCEPTO
                   ...
                   WHERE EMPL_SITUACION = ''I''
                     ...
                     AND TO_CHAR(X.EMPSOL_FECHA_VERIF, ''MM/YYYY'') = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'')
                     AND TO_CHAR(PDOC_FEC_GRAB, ''MM/YYYY'') = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'')
                     AND MES|| ''/'' || ANHO = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'')
                     UNION ALL
                     SELECT EMPL_LEGAJO,
                         EMPL_NOMBRE||'' ''||EMPL_APE,
   ...
   P_QUERY7:= 'SELECT EMPL_LEGAJO,NOMBRE,INGRESO, SALIDA, SUCURSAL, DEPARTAMENTO,
                 ...
                 --    AND EMPL_TIPO_DESV= ''DI''
                    -- AND B.PDDET_CLAVE_CONCEPTO(+) IN (25, 23,27)
                     ...
                     AND TO_CHAR(PDOC_FEC(+) , ''MM/YYYY'') = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
                       '||P_AND||'
                    --- AND mes|| ''/'' || ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''mm/YYYY'')
                     UNION ALL
                     SELECT EMPL_LEGAJO,
                         EMPL_NOMBRE||'' ''||EMPL_APE,
                         EMPL_FEC_INGRESO,
           ...
           /*P_QUERY:= 'SELECT
                          CASE
                             WHEN TO_CHAR(B.EMPL_FEC_SALIDA,''MM/YYYY'') = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -2),''MM/YYYY'') THEN
                               1
                             END CANT1,
                              TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -2),''MM/YYYY'') MES1,
                             CASE
                             WHEN TO_CHAR(B.EMPL_FEC_SALIDA,''MM/YYYY'') = TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'') THEN
                               1
                             END CANT2,
                             TO_CHAR(ADD_MONTHS(TO_DATE('''||P_FECHA||'''), -1),''MM/YYYY'') MES2,
                             CASE
                             WHEN TO_CHAR(B.EMPL_FEC_SALIDA,''MM/YYYY'') = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'') THEN
                               1
                             END CANT3,
                             TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')  MES3,
                           ...
                           B.TIPO_DESV,
                            ...
                            EMPL_TIPO_CONT,
                            TO_NUMBER(TO_CHAR(B.EMPL_FEC_SALIDA,''IW'')) SEM,
                            COUNT (B.TIPO_DESV) OVER(PARTITION BY TO_CHAR(B.EMPL_FEC_SALIDA,''IW''),(B.TIPO_DESV)) SEM_MOTV
                      FROM CUBO_EMPLEADO_V_DESV B, PER_EMPLEADO A
                      WHERE B.EMPL_LEGAJO = A.EMPL_LEGAJO
                      AND B.EMPL_EMPRESA = A.EMPL_EMPRESA
                      AND B.EMPL_FEC_SALIDA BETWEEN ''01/''||to_char(ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2), ''MM/YYYY'')  AND  TO_DATE('''||P_FECHA||''')
       ...
       /*  P_QUERY1 := 'SELECT CASE
                     ...
                     WHERE pddet_clave_doc = pdoc_clave
                       ...
                       AND B.PDDET_CLAVE_CONCEPTO IN (25, 23) AND
                       PDOC_FEC BETWEEN ''01/''||to_char(ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2), ''MM/YYYY'')  AND
                       TO_DATE('''||P_FECHA||''')
                       AND pddet_empr = 1
                       AND PDOC_EMPLEADO IN (SELECT C001
                                             WHERE collection_name = ''DESVINC_SALARIO''
                                               and c008 = ''Despido Injustificado''
                                               AND  C018 <>''T'')
                     GROUP BY TO_CHAR(PDOC_FEC, ''MM/YYYY'')';*/
...
P_QUERY1:='SELECT
     ...
     P_QUERY2 := 'SELECT TO_NUMBER(TO_CHAR(TO_DATE('''||P_FECHA||'''),''IW'')) SEMANA_1,
          ...
          /*'SELECT CASE
                     ...
                     WHERE pddet_clave_doc = pdoc_clave
                       and pddet_empr = pdoc_empr
                       AND B.PDDET_CLAVE_CONCEPTO IN (27)
                       AND   pddet_empr = 1
                       AND PDOC_FEC BETWEEN ''01/''||to_char(ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2), ''MM/YYYY'') AND
                       TO_DATE('''||P_FECHA||''')
                      AND
                       PDOC_EMPLEADO IN
                       (SELECT EMPL_LEGAJO
                          FROM CUBO_EMPLEADO_V_DESV
                         WHERE EMPL_FEC_SALIDA BETWEEN
                               ADD_MONTHS(TRUNC(TO_DATE('''||P_FECHA||'''), ''MM''), -2) AND
                               TO_DATE('''||P_FECHA||''')
                           AND EMPL_EMPRESA = 1)
             GROUP BY TO_CHAR(PDOC_FEC, ''MM/YYYY'')';*/
...
PROCEDURE PP_PROCESO_INDICE_ROTACION (P_EMPRESA NUMBER,
 ...
 IF  P_FECHA = LAST_DAY(TO_DATE(P_FECHA))  THEN
    P_AND := P_AND||'  MES || ''/'' || ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')  ';

 ELSE
   P_AND := P_AND||'  SEMANA || ''/'' || ANHO = TO_CHAR(TO_DATE('''||P_FECHA||'''),''IW/YYYY'')  ';
 END IF;
...
/*
     P_QUERY2 := 'SELECT
                    CASE
                      ...
                      WHEN DPTO_CODIGO =7 THEN
                         ''Taller''
                      WHEN DPTO_CODIGO IN (12,16) THEN
                        ''Logistica''
                       WHEN DPTO_CODIGO IN (9,10) THEN
                        ''Comercial''
                      WHEN DPTO_CODIGO = 11 THEN
...
P_QUERY6 := 'SELECT DPTO_CODIGO||''-''||DPTO_DESC DEPARTAMENTO,
  ...
  IF APEX_COLLECTION.COLLECTION_EXISTS(P_COLLECTION_NAME => 'INTER_ANUAL') THEN
    ...
    P_QUERY13 := 'SELECT T.EMPL_LEGAJO,
                        ...
                        -- (X.EMPSOL_FECHA_SAL_COP- EMPL_FEC_INGRESO)/30 DIAS,
                     ...
                     /*  CASE WHEN TO_CHAR(empsol_fecha_inc_per,''MM/YYYY'') =  '''||V_MES1||''' THEN
                        1
                      WHEN TO_CHAR(empsol_fecha_inc_per,''MM/YYYY'') =  '''||V_MES2||''' THEN
                        2
                      WHEN TO_CHAR(empsol_fecha_inc_per,''MM/YYYY'') =  '''||V_MES3||''' THEN
                        3
                      ELSE
                         4
                       END MAS_MESES*/
...
P_QUERY13 :='SELECT CASE
 ...
 delete x
  ...
  INSERT INTO X (CAMPO1, OTRO) VALUES (P_QUERY13, 'IND_ROTACION_TRANS');
 COMMIT;
...
values
 ...
 PROCEDURE ORDEN_MES (P_FECHA IN DATE) IS
   ...
   BEGIN
  -- RAISE_APPLICATION_ERROR (-20001, 'FFF');
...
IF TO_CHAR(SYSDATE,'DY','NLS_DATE_LANGUAGE=ENGLISH') = 'MON' AND TO_CHAR(SYSDATE, 'HH24:MI') >= '12:30'  THEN
 ...
 PROCEDURE PP_ESTRUCTURA_DETALLE (P_FECHA             IN DATE,
   ...
   V_SEMANA                        VARCHAR2(60);
 BEGIN
  -- RAISE_APPLICATION_ERROR(-20001, 'SIIII');
 /*    SELECT LAST_DAY(T.CONF_TABLEAU_DESDE), TO_CHAR(CONF_TABLEAU_DESDE,'MM/YYYY'),CONF_TABLEAU_DESDE
       INTO X_CONF_TABLEAU_HASTA, X_MES_CONF_TABLEU ,X_CONF_TABLEAU_DESDE
      FROM GEN_CONFIGURACION T
      WHERE T.CONF_EMPR = P_EMPRESA;
*/ ---ahora para el pb toma el mes actual menos 12
 ...
 /* P_QUERY7:= 'SELECT null FECHA,
        ...
        AND  FECHA BETWEEN '''|| V_PRIMER_DIA||''' AND  '''||P_FECHA||'''
...
/*
          APEX_COLLECTION.CREATE_OR_TRUNCATE_COLLECTION (
                      p_collection_name => 'DETALLE_VENTA');


   FOR  X IN (SELECT FECHA,
                     DOCU_NRO_DOC,
                     DETA_ART,
                     ART_DESC,
                     CANTIDAD_KG    CANTIDAD_KG,
                     VENTA_GS      VENTA_GS,
                     DECODE(SUC_DESC,'ASUNCION','CDA','CASA CENTRAL', 'CENTRAL',SUC_DESC) SUC,
                     ART_LINEA,
                     LINEA,
                     MES||'/'||ANHO ANHO
                FROM TBL_VENTA f
        WHERE ART_LINEA NOT IN (27,20)
        AND  FECHA BETWEEN  V_PRIMER_DIA AND  P_FECHA) LOOP


            APEX_COLLECTION.ADD_MEMBER(P_COLLECTION_NAME => 'DETALLE_VENTA',
                                       P_C001            => X.FECHA,
                                       P_C002            => X.DOCU_NRO_DOC,
                                       P_C003            => X.DETA_ART,
                                       P_C004            => X.CANTIDAD_KG,
                                       P_C005            => X.VENTA_GS,
                                       P_C006            => X.SUC,
                                       P_C007            => X.ART_LINEA,
                                       P_C008            => X.LINEA,
...
/*
 --------------------------DOTACION POR MES
 SELECT  SUM(MES1_VALOR) OVER(PARTITION BY MES1) MES1_TOTAL,
         SUM(MES2_VALOR) OVER(PARTITION BY MES2) MES2_TOTAL,
         SUM(MES3_VALOR) OVER(PARTITION BY MES3) MES3_TOTAL,
         SUM(PB) OVER(PARTITION BY MES_PB) PB_TOTAL
   INTO    V_DOT_MES1,V_DOT_MES2,V_DOT_MES3,V_DOT_PB
        FROM (SELECT  SUM(CASE WHEN EMPL_SITUACION = 'A' AND EMPL_FEC_INGRESO <= LAST_DAY(ADD_MONTHS(P_FECHA, -2)) THEN
                                        1
                               ELSE
                                        0
                               END) + SUM(CASE
                                               WHEN EMPL_SITUACION = 'I' AND EMPL_FEC_INGRESO <= LAST_DAY(ADD_MONTHS(P_FECHA, -2))
                                                  AND EMPL_FEC_SALIDA > LAST_DAY(ADD_MONTHS(P_FECHA, -2)) THEN
                                                   1
                                                  ELSE
                                                   0
                                                END) MES1_VALOR,
                          ...
                          SUM(CASE
                                 ...
                                 SUM(CASE
                                     ...
                                     END) + SUM(CASE
                                                ...
                                                TO_CHAR(ADD_MONTHS(P_FECHA, -2), 'MM/YYYY') MES1,
                                                TO_CHAR(ADD_MONTHS(P_FECHA, -1), 'MM/YYYY') MES2,
                                                TO_CHAR(P_FECHA, 'MM/YYYY') MES3,
                                                TO_CHAR(X_CONF_TABLEAU_DESDE, 'MM/YYYY') MES_PB,
                                   SUM(CASE
                                       WHEN EMPL_SITUACION = 'A' AND
                                            EMPL_FEC_INGRESO <= X_CONF_TABLEAU_HASTA THEN
                                        1
                                       ELSE
                                        0
                                     END) + SUM(CASE
                                                  WHEN EMPL_SITUACION = 'I' AND
                                                       EMPL_FEC_INGRESO <= X_CONF_TABLEAU_HASTA AND
                                                       EMPL_FEC_SALIDA > X_CONF_TABLEAU_HASTA THEN
                                                   1
                                                  ELSE
                                                   0
                                                END) PB
...
------------------------------------------------------------------------------------------
 --  RAISE_APPLICATION_ERROR (-20001, V_DOT_PB);
...
/*P_QUERY4 :='SELECT
        ...
        IF APEX_COLLECTION.COLLECTION_EXISTS(P_COLLECTION_NAME => 'MAYOR_INGRESO') THEN
         APEX_COLLECTION.DELETE_COLLECTION(P_COLLECTION_NAME => 'MAYOR_INGRESO');
      END IF;
         APEX_COLLECTION.CREATE_COLLECTION_FROM_QUERY_B(P_COLLECTION_NAME => 'MAYOR_INGRESO',
                                                         P_QUERY           => P_QUERY4);
*/
...
/*
----------------------------------------dotacion por mes a?o depende del la conf del tableau
IF APEX_COLLECTION.COLLECTION_EXISTS(P_COLLECTION_NAME => 'DOTACION_MESES') THEN
      APEX_COLLECTION.DELETE_COLLECTION(P_COLLECTION_NAME => 'DOTACION_MESES');
END IF;
      APEX_COLLECTION.CREATE_COLLECTION (P_COLLECTION_NAME => 'DOTACION_MESES');

   V_FECHA := X_CONF_TABLEAU_DESDE;

    WHILE TO_DATE(LAST_DAY(V_FECHA)) <= ADD_MONTHS(to_DATE(LAST_DAY(P_FECHA)),+1)  LOOP

          SELECT TO_CHAR(TO_DATE(V_FECHA), 'mm/yyyy') MES_ANHO,
                        SUM(CASE
                               WHEN EMPL_SITUACION = 'A' AND
                                    EMPL_FEC_INGRESO <= LAST_DAY(V_FECHA) THEN
                                1
      ...
      END IF;
     APEX_COLLECTION.ADD_MEMBER (
      p_collection_name => 'DOTACION_MESES',
                 p_C001 => P_MES_DOT,
                 P_C002 => P_MES_DOT_M);
...
/*
 SELECT  MAX(TOTAL) , MAX(TOTAL2)
  INTO P_OBJETIVO_GS, P_OBJETIVO_KG
  FROM (SELECT ROUND(TOTAL_GS/nvl(TOTAL_CANT,1)) TOTAL, ROUND(TOTAL_KG/nvl(TOTAL_CANT,1)) TOTAL2
          FROM (SELECT C002 MES_ANHO, TO_NUMBER(C001) TOTAL_CANT
                  FROM APEX_collections
                  WHERE collection_name = 'DOTACION_MESES') A,
                 (SELECT TO_NUMBER(C002) TOTAL_GS, C003 MES_ANHO, TO_NUMBER(C001) TOTAL_KG
                  FROM APEX_collections
                  WHERE collection_name = 'MAYOR_INGRESO') B
           WHERE A.MES_ANHO = B.MES_ANHO );
...
P_QUERY2 :='SELECT A.DOTACION, B.CANTIDAD_KG, B.VENTA_GS,SUC_DESC, VTA_OBJETIVO, CANT_OBJETIVO
               ...
               (SELECT CASE
                                   ...
                                   WHERE E.EMPL_DEPARTAMENTO = DPTO.DPTO_CODIGO
                                     ...
                                     AND EMPL_EMPRESA = 1
                                   '||P_AND||'
                                 --    AND MES || ''/'' || ANHO = ''09/2021''
                                    AND SEMANA||''/''||ANHO  =TO_CHAR(TO_DATE('''||P_FECHA||'''),''mm/YYYY'')
                                   --  AND EMPL_INCLUYE_TVC = ''S''
                                 GROUP BY CASE
                                         WHEN EMPL_SUCURSAL IN (2) AND DPTO.DPTO_CODIGO <> 31 THEN
                                          ''CDA''
                                    ...
                                    AND  C010 = TO_CHAR(TO_DATE('''||P_FECHA||'''),''MM/YYYY'')
                                     GROUP BY C007)B,/*
                                       (SELECT
                                         SUM(CAMPO5)    CANTIDAD_KG,
                                         SUM(CAMPO6)    VENTA_GS,
                                         CAMPO7 SUC_DESC
                                    FROM PERL051_EST_TEMP
                                     GROUP BY campo7)B,*/
     ...
     ELSE
        ...
        BEGIN
         ...
         WHERE EMPL_CODIGO_PROV <> 0
          ...
          /* AND ( MES||'/'||ANHO = TO_CHAR(TO_DATE(V_FECHA,'DD/MM/YYYY'),'MM/YYYY')
              OR SEMANA||'/'||ANHO  ='23/2020' /*CASE
                                       WHEN TO_CHAR(V_FECHA,'MM/YYYY') = TO_CHAR(TO_DATE(P_FECHA,'DD/MM/YYYY'),'MM/YYYY') THEN
                                         TO_CHAR(TO_DATE(P_FECHA,'DD/MM/YYYY'),'IW/YYYY')
                                       END)*/
           AND EMPL_FEC_INGRESO <= LAST_DAY(P_FECHA)
           AND EMPL_EMPRESA = 1
           GROUP BY  CASE WHEN EMPL_DEPARTAMENTO IN (3,4,5,6,7,8,13,21,23,24,27,28,33) THEN
...
P_QUERY1 := 'SELECT
 ...
 IF P_EMPRESA =2 THEN
  ...
  delete from x
  where otro = 'DOT_CANTIDAD111';
    insert into x
      (campo1, otro)
    values
      (P_QUERY1, 'DOT_CANTIDAD111');
