create or replace procedure PP_ENVIAR_MAIL_PERI_ANT_FIN(P_CLAVE       IN NUMBER DEFAULT NULL,
...
BEGIN
  ...
  if DESTINATARIO is not null then
    -- 01/07/2022 11:14:29 @PabloACespedes \(^-^)/
    -- se_retiro SYS.UTL_MAIL >> UTL_MAIL
    UTL_MAIL.SEND(sender     => 'noreply',
                      ...
                      message    => saludo || '.
Se a ' || TIPO ||
                                     ' Una Factura de Finanzas del Mes Anterior Tenga Cuidado,Para Que No Haya Diferencia.' ||
                                     ' Nro de Documento: ' || P_CLAVE || ' Fecha Ingresada : ' || FECHA ||' Fecha Afectada: ' || D_FEC_EMISION ||
                                     ' Usuario: ' ||V_USER,
...
end PP_ENVIAR_MAIL_PERI_ANT_FIN;
/
