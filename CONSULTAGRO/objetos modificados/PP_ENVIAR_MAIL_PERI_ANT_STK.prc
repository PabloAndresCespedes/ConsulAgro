create or replace procedure PP_ENVIAR_MAIL_PERI_ANT_STK(P_CLAVE       IN NUMBER DEFAULT NULL,
...
BEGIN
  ...
  if DESTINATARIO is not null then
    -- 01/07/2022 11:16:45 @PabloACespedes \(^-^)/
    ...
    UTL_MAIL.SEND(sender     => 'noreply',
                      ...
                      message    => saludo || '.
Se a ' || TIPO ||
                                     ' Una Factura de Stock del Mes Anterior Tenga Cuidado,Para Que No Haya Diferencia.' ||
                                     ' Nro de Documento: ' || P_CLAVE || ' Fecha de Ingreso: ' || FECHA || ' Fecha Afectada: ' || D_FEC_EMISION ||
                                     ' Usuario:' || V_USER ,
...
end PP_ENVIAR_MAIL_PERI_ANT_STK;
/
