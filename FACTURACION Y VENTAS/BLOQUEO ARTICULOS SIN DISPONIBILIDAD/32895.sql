create or replace PACKAGE FACI240_P IS
  ...
  function get_disp_venta(in_art  in  number) return number;
...
create or replace PACKAGE BODY FACI240_P IS
  ...
  --===========================================================================
  function get_disp_venta(in_art  in  number) return number is
    l_r number;
  begin
    select t.DISP_VENTA
    into l_r
    from V_LISTADO_STKL276 t
    where t.ART_CODIGO = in_art;
  ...
  l_existencia number;
  begin
    l_existencia := get_disp_venta(in_art => AP.V( 'P48_ART_CODIGO' ));
    if l_existencia < AP.GETNC('P48_CANT_PED') then
      raise_application_error(-20001, 'No puede cargar m·s de lo disponible para venta. Total Disponible: '||l_existencia);
    end if;
  end;
  --======================================================================================        
  ...
  BEGIN
       ...
       IF FP_EXIS_ARTICULO( V_EMPRESA, V_ARTICULO ) THEN 
           ...
           AP.SV('P48_CANT_DISP_VENTA'  , get_disp_venta(in_art => V_ARTICULO) ) ; 
