...
CREATE OR REPLACE PACKAGE BODY FACM070 IS
    ...
    CURSOR CUR_LIST(IC_NRO_LISTA IN number /*varchar2*/) IS --fas
      ...
      FROM \*(SELECT *
       ...
       WHERE LIPR_NRO_LISTA_PRECIO(+) = IC_NRO_LISTA
         ...
         and (in_grupo_art is null or sa.art_grupo = in_grupo_art)
  ...
  begin
    apex_collection.create_or_truncate_collection(p_collection_name => co_col_lista);
    
    for c in (
      select de.lipr_art              art_cod,
             de.lipr_art_alfanumerico art_alfa,
             h.liprh_precio_anterior  precio_unitario,
             h.liprh_precio_anterior  precio_actual,
             de.lipr_precio_unitario  precio_anterior,
             h.liprh_fecha_cambio     fecha_cambio,
             h.liprh_empr             empresa,
             de.lipr_linea            linea,
             de.lipr_grupo            grupo,
             h.liprh_nro_lista_precio nro_lista,
             a.art_cod_barra          cod_barra,
             a.art_desc               art_desc,
             de.lipr_suc              sucursal,
             f.lipe_desc              lista_precio_desc
      from fac_lista_precio_det_hist h
      inner join fac_lista_precio f on (f.lipe_nro_lista_precio = h.liprh_nro_lista_precio)
      inner join fac_lista_precio_det de on (de.lipr_nro_lista_precio = h.liprh_nro_lista_precio and de.lipr_art = h.liprh_art)
    ...
    loop
      apex_collection.add_member(p_collection_name => co_col_lista,
                                 p_c001            => c.art_cod,
                                 p_c002            => c.art_alfa,
                                 p_c003            => c.precio_unitario,
                                 p_c004            => nvl(c.precio_anterior, 0),
                                 p_c005            => nvl(c.precio_unitario, 0),
                                 p_c006            => in_nro_lista,
                                 p_c007            => c.empresa,
                                 p_c008            => c.linea,
                                 p_c009            => c.grupo,
