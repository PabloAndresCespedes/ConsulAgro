...
create or replace package body FINI053 is
    ...
    case
      when l_ind_er in( co_emision, co_recibido ) then
        ...
        */
        ...
        if l_fc_tipo_mov not in( l_tmv_fc_cr_emit , l_tmv_ndb_emit, l_tmv_fc_cr_rec, l_tmv_nota_deb_rec, l_tmv_fc_cr_rec_ajus )
        then
  ...
  exception
    when e_tmv_no_configurado_one then
      raise_application_Error(-20000, '1. No se puede cancelar el documento, no se encuentra configurado. Doc. Tipo Mov. '||l_tmv_cancela);
    when e_tmv_no_configurado_two then
      raise_application_Error(-20000, '2. No se puede cancelar el documento, no se encuentra configurado. Doc. Tipo Mov. '||l_tmv_cancela_two);
