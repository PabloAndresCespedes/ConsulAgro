...
CREATE OR REPLACE PACKAGE BODY FACI244 IS
 ...
 --
  ...
  PROCEDURE PP_ACT_REL_FIN_STK(I_DOC_NRO_PCOMPRA_FLOTA IN NUMBER) IS
    V_CLAVE_FIN NUMBER := TFIN_DOCUMENTO.DOC_CLAVE;
  BEGIN
    NULL;
    IF I_DOC_NRO_PCOMPRA_FLOTA IS NOT NULL THEN
      ...
      /*
      LOOP
        IF :TRAS_AGREGAR <> 'N' THEN
          INSERT INTO FIN_DOC_STK_DOC
            (FSD_CLAVE_FIN, FSD_CLAVE_STK, FSD_EMPR)
          VALUES
            (V_CLAVE_FIN, :TRAS_CLAVE, I_EMPRESA);
        END IF;
       END LOOP;*/
    END IF;
 ...
 --  raise_application_error(-20001, TFIN_DOCUMENTO.DOC_NETO_EXEN_MON||' '||TFIN_DOCUMENTO.DOC_NETO_GRAV_MON|| TFIN_DOCUMENTO.DOC_IVA_MON);
  ...
  BEGIN
    APEX_COLLECTION.DELETE_MEMBER(P_COLLECTION_NAME => 'FACI244_DET',
                                  P_SEQ             => I_SEQ);

    APEX_COLLECTION.RESEQUENCE_COLLECTION(P_COLLECTION_NAME => 'FACI244_DET');
  ...
  BEGIN
    ...
    PP_ACT_REL_FIN_STK(I_DOC_NRO_PCOMPRA_FLOTA => I_DOC_NRO_PCOMPRA_FLOTA);
...
-----------------------------------------------------------------------------------------
  ...
  co_col_cab constant varchar2(22 char) := 'COMBUSTIBLES_PETROBRAS';
  co_col_det constant varchar2(26 char) := 'COMBUSTIBLES_PETROBRAS_DET';
  ...
  begin
    ...
    apex_collection.create_or_truncate_collection(p_collection_name => co_col_cab);
    apex_collection.create_or_truncate_collection(p_collection_name => co_col_det);
    ...
    loop
      apex_collection.add_member(p_collection_name => co_col_cab,
